...
## Question 1: API Wrapper Design (20 minutes)
...
## Question 2: Data Type Conversion (15 minutes)

### Scenario
You're building a system that receives data from a sensor in this format:

```python
sensor_data = b'\x00\x00\x00\x05\x00\x00\x00\x03\x41\x48\x00\x00'
# This represents: id=5, count=3, temperature=12.5
```

The data is packed as:
- Bytes 0-3: Device ID (4-byte integer)
- Bytes 4-7: Sample count (4-byte integer)  
- Bytes 8-11: Temperature (4-byte float)

### Task
Write a function to unpack this binary data into a Python dictionary:
```python
{
    'device_id': 5,
    'sample_count': 3,
...
### What This Tests
- **Binary data handling** - exactly what you do with DLLs
- **Struct packing/unpacking** - core DLL skill
- **Byte order awareness** - critical for cross-platform DLL work
- **Data type understanding** - must understand int vs float vs bytes
...
def unpack_sensor_data(data):
    """
    Unpack binary sensor data
    Format: 
    - 4 bytes: device ID (integer)
    - 4 bytes: sample count (integer)
    - 4 bytes: temperature (float)
    """
    if len(data) != 12:
        raise ValueError(f"Expected 12 bytes, got {len(data)}")
    
    # Unpack: I=unsigned int, f=float, >=big-endian
    device_id, sample_count, temperature = struct.unpack('>IIf', data)
...
# Test it
sensor_data = b'\x00\x00\x00\x05\x00\x00\x00\x03\x41\x48\x00\x00'
result = unpack_sensor_data(sensor_data)
print(result)
```

### Follow-up Questions
- "What's the difference between big-endian and little-endian?"
- "How would you handle variable-length data?"
- "What if the temperature could be negative?"
- "How would you pack data to send back to the sensor?"

### Red Flags
- Tries to parse manually byte by byte (shows no knowledge of struct)
- Doesn't consider byte order
- No error checking for data length
- Doesn't understand data types

### Green Flags
- Uses struct module
- Validates input length
- Asks about byte order
- Can explain format string
- Considers edge cases

---

## Question 3: Simple REST API (25 minutes)
